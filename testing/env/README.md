...
The environment is controlled by docker-compose, but there are three programs that simplify the usage of docker-compose. `build` can be used to create all of the images. `clean` can be used to delete all of the images. Finally, `controller` can be used to bring up or tear down the grid. After its action, `controller` accepts compose files to apply over `docker-compose.yml`, such as `docker-compose.provider-alma9.yml`, which swaps in an AlmaLinux 9 catalog service provider running iRODS `IRODS_VERSION`; pass the same files to `stop` as to `start`.
